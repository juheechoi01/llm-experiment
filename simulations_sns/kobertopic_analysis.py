...
def label_topics_gpt(topic_info: dict) -> dict[int, str]:
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": (
                "아래는 미성년자 소셜 미디어 사용 제한 법안 관련 논증 문장들의 토픽 클러스터입니다.\n"
...
def visualize(topic_model, texts, embs_norm, records, topic_ids, topic_info,
    ...
    try:
        fig = topic_model.visualize_documents(
            ...
            title="KoBERTopic — Point Statements [simulations_sns]",
        )
    ...
    try:
        ...
        fig_cond.update_layout(
            title=dict(
                text=(f"KoBERTopic — 조건별 문서 분포 [simulations_sns]<br>"
    ...
    try:
        ...
        fig_bar.update_layout(
            barmode="group",
            title=f"KoBERTopic 토픽별 조건 분포 [simulations_sns]<br><sup>{div_lines}</sup>",
            yaxis_title="비율 (%)",
            xaxis_tickangle=-25,
