#!/usr/bin/env python3
"""
KoBERTopic 분석 (simulation_sns)
  - 데이터: authorless-2.jsonl / tangible-2.jsonl
...
def _para_transition(content):
    """transition marker 기준 분리 → 최대 3 논거."""
    segs = _TRANS_SPLIT.split(content.strip())
    if len(segs) < 2:
        return {}
    points = {}
    # 첫 segment: 첫 문장(intro/stance) 제외, 두 번째 문장이 arg1
...
def parse_response(content):
    ...
    if not points:
        ...
        for i, text in enumerate(paras[1:4], 1):
            stmt, sub = _split_stmt_sub(text)
            points[i] = {"statement": stmt, "subclaim": sub}
    # 4) transition marker 기반 단락 분리
    if not points:
        points = _para_transition(content)
    # 5) 문장 수준 heuristic (transition 없는 단락)
    if not points:
...
def label_topics_gpt(topic_info: dict) -> dict[int, str]:
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": (
                "아래는 미성년자 SNS 법안 반대 논증 문장들의 토픽 클러스터입니다.\n"
...
def run_kobertopic(texts, embs_norm, records):
    ...
    for tid in sorted(set(topic_ids)):
        ...
        t_cnt = sum(1 for r in recs if r["condition"] == COND_T)
        a_cnt = sum(1 for r in recs if r["condition"] == COND_A)
...
def print_results(records, topic_info, n_topics, n_noise, topic_ids):
    ...
    expected_t   = sum(1 for r in records if r["condition"] == COND_T) / len(records)
    ...
    print(f"{'='*70}")
    print(f"기준선: tangible-2 {expected_t*100:.0f}% / authorless-2 {(1-expected_t)*100:.0f}%\n")
...
def visualize(topic_model, texts, embs_norm, records, topic_ids, topic_info,
    ...
    try:
        fig = topic_model.visualize_documents(
            ...
            title="KoBERTopic — Point Statements [simulation_sns]",
        )
    ...
    try:
        ...
        fig_cond = make_subplots(
            rows=1, cols=2,
            subplot_titles=["Authorless-2", "Tangible-2"],
            horizontal_spacing=0.08,
        )

        for col, cond in enumerate([COND_A, COND_T], start=1):
        ...
        fig_cond.update_layout(
            title=dict(
                text=(f"KoBERTopic — 조건별 문서 분포 [simulation_sns]<br>"
    ...
    try:
        ...
        fig_bar = go.Figure(data=[
            go.Bar(name="Tangible-2",   x=xlabels, y=t_pcts, marker_color="#E15759"),
            go.Bar(name="Authorless-2", x=xlabels, y=a_pcts, marker_color="#4E79A7"),
        ])
        fig_bar.add_hline(y=expected_t * 100, line_dash="dash", line_color="gray",
                          annotation_text=f"기준선 {expected_t*100:.0f}%")
        fig_bar.update_layout(
            barmode="group",
            title=f"KoBERTopic 토픽별 조건 분포 [simulation_sns]  (KL={kl:.4f}, JSD={jsd:.4f})",
            yaxis_title="비율 (%)",
            xaxis_tickangle=-25,
...
def main():
    print("데이터 로딩...")
    authorless = load_jsonl(DATA_DIR / "authorless-2.jsonl")
    tangible   = load_jsonl(DATA_DIR / "tangible-2.jsonl")
    ...
    total_t = sum(1 for r in records if r["condition"] == COND_T)
    total_a = sum(1 for r in records if r["condition"] == COND_A)
    print(f"  총 {len(texts)}개 statements  (tangible-2 {total_t} / authorless-2 {total_a})")
